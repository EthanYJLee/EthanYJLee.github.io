...
const Work = () => {
  ...
  const renderMedia = (project) => {
    if (project.id === "crackdetector") {
      return (
        <video src="videos/crack_detector.mp4" style={videoStyle} controls muted loop preload="metadata" playsInline>
          Your browser does not support the video tag.
        </video>
      );
    } else if (project.img) {
      return <img src={project.img} alt={project.title} loading="lazy" style={imageStyle} />;
    } else {
      return (
          ...
          <div className="project-navigation">
            ...
            <div className="indicator">
              {filteredProjects.map((_, index) => (
                <span key={index} className={`dot ${index === activeIndex ? "active" : ""}`} onClick={() => handleDotClick(index)} />
              ))}
            </div>
                  ...
                  <>
                    <div className="text-section">
                      <div className="title-row">
                        ...
                        <span className={`project-tag ${project.category}`}>{t(`work.tag.${project.category}`)}</span>
                      </div>
                      {project.subheading && <p className="subheading">{project.subheading}</p>}
                      ...
                      <div className="btn">
                        {project.btn.map((btnInfo, i) => (
                          <a key={i} href={btnInfo.link || "#"} target="_blank" rel="noopener noreferrer">
                            {btnInfo.text}
                          </a>
                        ))}
                      </div>
                    </div>
                    <div className="image-section">{renderMedia(project)}</div>
                  </>
                ) : (
                  <>
                    <div className="image-section">{renderMedia(project)}</div>
